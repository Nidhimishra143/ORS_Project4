...
public class RoleModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("SELECT MAX(ID) FROM demo_ors.st_role");
	...
	public long add(RoleBean bean) throws ApplicationException,
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("INSERT INTO demo_ors.st_role VALUES(?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
	...
	public void delete(RoleBean bean) throws ApplicationException {
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("DELETE FROM demo_ors.st_role WHERE ID=?");
	...
	public RoleBean findByName(String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM demo_ors.st_role WHERE NAME=?");
		RoleBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public void update(RoleBean bean) throws ApplicationException,
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("UPDATE demo_ors.st_role SET NAME=?,DESCRIPTION=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
