...
public class UserModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("SELECT MAX(ID) FROM demo_ors.st_user");
	...
	public long add(UserBean bean) throws ApplicationException,
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("INSERT INTO demo_ors.st_user VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
			pstmt.setString(2, bean.getFirstName());
			pstmt.setString(3, bean.getLastName());
			pstmt.setString(4, bean.getLogin());
			pstmt.setString(5, bean.getPassword());
			// pstmt.setDate(6, (bean.getDob().getTime()));
	...
	public void delete(UserBean bean) throws ApplicationException {
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("DELETE FROM demo_ors.st_user WHERE ID=?");
	...
	public UserBean findByLogin(String login) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM demo_ors.st_user WHERE LOGIN=?");
		UserBean bean = null;
		Connection conn = null;
	...
	public UserBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM demo_ors.st_user WHERE ID=?");
		UserBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public void update(UserBean bean) throws ApplicationException,
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("UPDATE demo_ors.st_user SET FIRST_NAME=?,LAST_NAME=?,LOGIN=?,PASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,UNSUCCESSFUL_LOGIN=?,GENDER=?,LAST_LOGIN=?,USER_LOCK=?,REGISTERED_IP=?,LAST_LOGIN_IP=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
	...
	public List search(UserBean bean, int pageNo, int pageSize)
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM demo_ors.st_user WHERE 1=1");
		...
		if (bean != null) {
			...
			if (bean.getRegisterdIp() != null
					&& bean.getRegisterdIp().length() > 0) {
				sql.append(" AND REGISTERED_IP LIKE '" + bean.getRegisterdIp()
						+ "%'");
			}
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer("select * from demo_ors.st_user");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
	...
	public UserBean authenticate(String login, String password)
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM demo_ors.st_user WHERE LOGIN = ? AND PASSWORD = ?");
		UserBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public List getRoles(UserBean bean) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM demo_ors.st_user WHERE role_Id=?");
		Connection conn = null;
