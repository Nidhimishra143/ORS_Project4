...
public class MarksheetModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("select max(ID) from demo_ors.st_marksheet");
	...
	public long add(MarksheetBean bean) throws ApplicationException,
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("INSERT INTO demo_ors.st_marksheet VALUES(?,?,?,?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
	...
	public void delete(MarksheetBean bean) throws ApplicationException {
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("DELETE FROM demo_ors.st_marksheet WHERE ID=?");
	...
	public MarksheetBean findByRollNo(String rollNo)
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM demo_ors.st_marksheet WHERE ROLL_NO=?");
	...
	public MarksheetBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM demo_ors.st_marksheet WHERE ID=?");
		MarksheetBean bean = null;
		Connection conn = null;
		try {
	...
	public void update(MarksheetBean bean) throws ApplicationException,
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("UPDATE demo_ors.st_marksheet SET ROLL_NO=?,STUDENT_ID=?,NAME=?,PHYSICS=?,CHEMISTRY=?,MATHS=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
	...
	public List search(MarksheetBean bean, int pageNo, int pageSize)
		...
		StringBuffer sql = new StringBuffer(
				"select * from demo_ors.st_marksheet where true");
		if (bean != null) {
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer("select * from demo_ors.st_marksheet");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
	...
	public List getMeritList(int pageNo, int pageSize)
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer(
				"SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMISTRY`, `MATHS` , (PHYSICS + CHEMISTRY + MATHS) as total from demo_ors.st_marksheet order by total desc");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
